import { fetchAndSyncPlaces } from "@/domain/discovery/googlePlaces.service";
import { prisma } from "@/lib/db/prisma";

const BANGKOK_CITY_ID = "bangkok-th";
const INTERESTS_TO_SYNC = [
  "rooftop_bars",
  "street_food_markets",
  "coworking_spaces",
];
  ...
  try {
    ...
    for (const interestId of INTERESTS_TO_SYNC) {
      try {
        ...
        const syncResult = await fetchAndSyncPlaces(
          BANGKOK_CITY_ID,
          interestId,
        );
