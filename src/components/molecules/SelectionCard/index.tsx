...
const selectionCardVariants = cva(
  "group flex items-center rounded-md p-sm cursor-pointer border transition-all duration-200 outline-none",
  {
    variants: {
      isSelected: {
        true: "border-brand bg-brand/10 shadow-sm",
        false: "border-surface hover:border-brand/30 shadow-secondary",
      },
    },
    defaultVariants: {
      isSelected: false,
...
const Indicator = ({
  ...
  if (icon) {
    return (
      <div
        className={cn(
          "p-md rounded-sm transition-colors duration-200 flex items-center justify-center",
          isSelected
            ? "bg-brand text-white border-2 border-brand"
            : "bg-surface-secondary text-txt-sec border border-transparent",
        )}
      >
        {icon}
      </div>
  ...
  return (
    <div
      aria-hidden="true"
      className={cn(
        "relative h-5 w-5 flex shrink-0 items-center justify-center transition-all duration-200 border",
        shape,
        isSelected
