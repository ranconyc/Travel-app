  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.error || "Failed to save country");
      }
    } catch (e) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-surface rounded-xl p-6 shadow-sm border border-border">
            ...
            <div className="grid grid-cols-1 gap-md">
              ...
              <div className="col-span-1">
                ...
                <input
                  value={data.cca3 || ""}
                  onChange={(e) => setData({ ...data, cca3: e.target.value })}
                  className="w-full p-2 bg-surface border border-surface-secondary rounded focus:border-brand outline-none transition-colors"
                />
              </div>
          ...
          <div className="bg-surface rounded-xl p-6 shadow-sm border border-border">
            <h2 className="text-lg font-bold mb-md">Regions</h2>
            <ArrayFieldEditor
              label="Regions"
              items={data.regions}
              onChange={(newItems) => setData({ ...data, regions: newItems })}
            />
          </div>
