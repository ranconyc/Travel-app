...
import MoodSelector, { Mood, getMoodRelatedInterests } from "@/components/molecules/MoodSelector";
...
export default function DiscoveryHome() {
  ...
  const [userLocation, setUserLocation] = useState<typeof BANGKOK_COORDS | null>(null);
  const user = useUser();
  ...
  const placesWithDistance = useMemo(() => {
    return filteredPlaces.map(({ place, matchResult }) => ({
      place,
      matchResult,
      distance: userLocation && place.coords 
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <Typography variant="tiny" className="text-gray-600 dark:text-gray-400">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {placesWithDistance.map(({ place, matchResult, distance }) => (
              <PlaceCard
                ...
                distance={distance}
                priority={matchResult.finalScore >= 80}
              />
            ))}
          </div>
        )}
