...
const LicensePlate = ({ countryCode }: { countryCode: string }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <div
        className={`w-12 h-6 rounded flex items-center justify-center text-xs font-bold ${isEU ? "bg-blue-600 text-inverse" : "bg-gray-200 text-gray-700"}`}
      >
...
export default function LogisticsSection() {
  ...
  const safeTimezones = Array.isArray(formattedTimezones)
    ? formattedTimezones
    : [];
  const safePlugTypes = Array.isArray(plugTypes) ? plugTypes : [];

  return (
    <div className="px-4 py-6 space-y-6 bg-surface rounded-3xl border border-surface-secondary shrink-0 w-[312px]">
      {/* Header */}
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap gap-2">
            {safeTimezones.map((tz, index) => (
              <span
                key={index}
                className="px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded-full text-ui-sm text-gray-700 dark:text-gray-300"
              >
                {tz}
              </span>
            ))}
          ...
          <div className="pt-2 border-t border-gray-200 dark:border-gray-600">
            <p className="text-caption dark:text-gray-400">
              <span className="font-medium">Start of the week:</span>{" "}
              {data.startOfWeek || "Monday"}
            </p>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <p className="text-sm text-gray-700 dark:text-gray-300">
                Drive on the{" "}
                <span className="font-medium">{data.car?.side || "Right"}</span>
              </p>
