 ...
 */
export async function fetchAndSyncPlaces(cityId: string, interestId: string) {
  ...
  if (apiLockService.wasRecentlySynced(lockKey)) {
    console.log(
      `🔒 Skipping sync for ${cityId}:${interestId} - recently synced`,
    );
  ...
  try {
    ...
    const { getCityById } = await import("@/lib/db/cityLocation.repo");
    ...
    const { countPlaces, upsertPlace } = await import("@/lib/db/place.repo");

    const existingPlacesCount = await countPlaces({
      cityRefId: city.id,
      tags: { has: interestId },
      updatedAt: {
    ...
    if (existingPlacesCount >= 20) {
      console.log(
        `📊 Sufficient recent data exists for ${cityId}:${interestId} (${existingPlacesCount} places)`,
      );
    ...
    console.log(
      `✅ Sync completed for ${cityId}:${interestId} - ${result.count} places`,
    );
